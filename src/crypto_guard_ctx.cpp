...
struct CryptoGuardCtx::Impl {
    ...
    void ThrowErrorOpenSSL(std::string_view description) const {
        throw std::runtime_error(std::format("OpenSSL error code: {}", ERR_get_error()));
    }
    ...
    void Process(Operation type, std::iostream &inStream, std::iostream &outStream, std::string_view password) const {
        ...
        if (!EVP_CipherInit_ex(ctx.get(), params.cipher, nullptr, params.key.data(), params.iv.data(),
                               params.encrypt)) {
            ThrowErrorOpenSSL("Cipher initialization failed");
        }
        ...
        while (!inStream.eof()) {
            ...
            if (inStream.bad()) {
                throw std::runtime_error("Error occured while reading input file!");
            }
            if (!EVP_CipherUpdate(ctx.get(), outBuf.data(), &outlen, inBuf.data(), inlen)) {
                ThrowErrorOpenSSL("CipherUpdate");
            }
            outStream.write((char *)outBuf.data(), outlen);
            if (not outStream) {
                throw std::runtime_error("Error occured while writing output file!");
            }
        }
        if (!EVP_CipherFinal_ex(ctx.get(), outBuf.data(), &outlen)) {
            ThrowErrorOpenSSL("Cipher finalization failed.");
        }
        outStream.write((char *)outBuf.data(), outlen);
        if (not outStream) {
            throw std::runtime_error("Error occured while writing output file!");
    ...
    std::string CalculateChecksum(std::iostream &inStream) const {
        ...
        if (md == NULL) {
            ThrowErrorOpenSSL("Invalid digest!");
        }
        MdCtxGuard mdctx{EVP_MD_CTX_new()};
        if (not mdctx) {
            ThrowErrorOpenSSL("Message digest create failed");
        }
        if (!EVP_DigestInit_ex2(mdctx.get(), md, NULL)) {
            ThrowErrorOpenSSL("Message digest initialization failed");
        }
        while (!inStream.eof()) {
            inStream.read((char *)inBuf.data(), bufferSize);
            if (inStream.bad()) {
            ...
            if (!EVP_DigestUpdate(mdctx.get(), inBuf.data(), inlen)) {
                ThrowErrorOpenSSL("Message digest update failed");
            }
        }
        if (!EVP_DigestFinal_ex(mdctx.get(), md_value, &md_len)) {
            ThrowErrorOpenSSL("Message digest finalization failed.");
        }
        std::stringstream ss;
    ...
    AesCipherParams CreateChiperParamsFromPassword(std::string_view password) const {
        ...
        if (result == 0) {
            ThrowErrorOpenSSL("Failed to create a key from password");
        }
